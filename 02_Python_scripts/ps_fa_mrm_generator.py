...
def mrm_output(fa1):
    ...
    while fa_desat_no_mass != []:
        for i in fa_desat_no_mass:
            ps_desat_mass += [fa_desat_no_mass[0] + i]

        fa_desat_no_mass.remove(fa_desat_no_mass[0])
            


    # to output the ps id (i.e. PS-(fa C no.):(fa desat no.))
    ps_id = []
    ps_M1_mass = []

    id_counter = len(ps_carbon)
    id_number = len(ps_carbon)

    
    while id_counter != 0:
        ps_id += ["PS-" + str(ps_carbon[id_number - id_counter]) + ":" + str(ps_desat[id_number - id_counter])]
        
        ps_M1_mass += [(carbon * (ps_carbon[id_number - id_counter] + 6)) + (hydrogen * ((ps_carbon[id_number - id_counter] * 2) - (ps_desat_mass[id_number - id_counter] * 2) + 10)) + (oxygen * 10) + phosphorus + nitrogen - hydrogen]

        id_counter -= 1
    


    # to output the FA combinations for each PS (this is the same as the original TAG/FA combinator)
    ps = []
    ...
    all_ps_id = ps_id + ps_id
    all_ps_M1_mass = ps_M1_mass + ps_M1_mass
    all_ps_FA_label = []

    ps_FA_label_counter = len(all_ps_id)
    ps_FA_label_number = len(all_ps_id)

    for i in all_ps_id:
        all_ps_FA_label += [i + '-FA' + all_fatty_acids[ps_FA_label_number - ps_FA_label_counter]]

        ps_FA_label_counter -= 1



    MRM_table = pd.DataFrame({'PS ID': ps_id, 'FA composition': ps, 'M1 mass (-H)': ps_M1_mass, 'M3 -FA1 mass': M3_fa1, 'M3 -FA2 mass': M3_fa2})
    print(MRM_table)
    MRM_table.to_csv('MRM_table.csv')
 

    MRM_condensed_table = pd.DataFrame({'PS ID': all_ps_id, 'FA loss': all_fatty_acids, 'PS-FA label': all_ps_FA_label, 'M1 mass (-H)': all_ps_M1_mass, 'M3 -FA mass': all_M3_transitions})
